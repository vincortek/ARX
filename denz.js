...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:デ ニ ス\n'
            + 'ORG:Owner VBott;\n'
            + 'TEL;type=CELL;type=VOICE;waid=+62 812 2675 6031:+62 858-66295-942\n'
            + 'END:VCARD'
...
memberLimit = 10
ban = []
premium = ["+62 812 2675 6031@s.whatsapp.net","6285876210829@s.whatsapp.net"]
...
ᴋᴇᴛɪᴋ ${prefix}daftar ᴜɴᴛᴜᴋ ᴍᴇᴍᴜʟᴀɪ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ʙᴏᴛ`
	...
	denz.on('chat-update', async (mek) => {
		try {
			...
			const DnsBot = ["+62 812 2675 6031@s.whatsapp.net"] // ubah aja gapapa
			...
			const ownerNumber = ["+62 812 2675 6031@s.whatsapp.net","6285876210829@s.whataapp.net"] // owner number ubah aja
...
denz.sendMessage(from, hasil, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "ketik #menu kak", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
		...
		reply(`*_「 ʙᴀᴅᴡᴏʀᴅ ᴅᴇᴛᴇᴄᴛᴇᴅ 」_*\nᴍᴀᴀғ *${pushname2}* ᴀɴᴅᴀ ʙᴇʀʙɪᴄᴀʀᴀ ᴋᴏᴛᴏʀ!, ᴀɴᴅᴀ ꜱᴇɢᴇʀᴀ ᴅɪᴋɪᴄᴋ ᴅᴀʀɪ ɢʀᴜᴘ *${groupMetadata.subject}*`)
			...
			if (!isUser) return reply(mess.only.userB)
				dmenu = `╓───「 𝓥𝓲𝓷𝓷𝓬𝓸𝓻𝓮 」
║wa.me/+62 812 2675 6031
╙───々


MASIH ADA BEBERAPAYANG GABISA DIGUNAKAN, JADI KALO LU NYURUH GUA BUAT BENERIN , KAPAN KAPAN ANJG LU KIRA SEMUDAH MASUKIN BATANG KE BOLONGAN
...
╓───「 *_𝓥𝓲𝓷𝓷𝓬𝓸𝓻𝓮_* 」
║Follow ig :@_vinn3
╙───々`buat cewe. ati ati kalo lu genit reva ngamok
...
denz.sendMessage(from, brando, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true}) 	    
            ...
            case 'help':
brando = await getBuffer(`https://pencarikode.xyz/api/cita-cita?apikey=pais`, {method: 'get'})                
denz.sendMessage(from, help(name), text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "@_vinn3", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
denz.sendMessage(from, brando, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
    	    break
            case 'gifstiker':
				case 'stiker':
...
case 'igpost': 				
				...
				buffer = await getBuffer(anu.result.url)
                                denz.sendMessage(from, buffer, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "@_vinn3", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('dnsnew.jpg')} } }})
				break
		case 'fakeaddress': 
				reply(`[❕] Loading`)
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
         ...
         case 'lirik':
                ...
                if (args.length < 1) return reply(`Yang mau direport apaan? Contoh: #report fitur #hentai error`)
                    ...
                    denz.sendMessage('+62 812 2675 6031@s.whatsapp.net', options, text, {quoted: mek})
...
quoted: mek, caption: `Hasil Dari *${daerah}*`
  ...
  break
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 *VBott* 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 *VBott* 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat VBott\`\`\``)
					break
                                case 'bcgc':
					denz.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
...
case 'husbu':
        ...
        if (budy.includes("+62 812 2675 6031")){  // respon tag ubah aja
